# 🏙️ Forecasting Project — From Potential to Performance: Using AI Algorithms in Urban Development
...
- 🏗️ Infrastructure and technology are the dominant predictors of Smart City ranking, overriding general human development metrics.
...
### 1️⃣ Model 1 — Random Forest (Classification)

- 🎯 **Goal:** Classify cities into Smart City rating classes and predict Bucharest's class.
- ⚙️ **Implementation:** 700 trees; qualitative variables encoded as ordered factors.
- 📊 **Results:**
...
### 2️⃣ Model 2 — Stepwise Regression (Variable Selection)

- 🎯 **Goal:** Identify the minimal set of variables that best predict the numerical ranking (SMCR_2024).
- ⚙️ **Method:** `stepAIC` with both forward and backward selection (R, `MASS` package).
...
### 3️⃣ Model 3 — Random Forest v2.0 (Scenario Simulation)
...
- 🏗️ **Infrastructure (Structure2024) is by far the most influential predictor** of Smart City ranking.
...
- ⚠️ The gap between Bucharest's potential (estimated rank ~78) and actual rank (100) reflects deficiencies in urban mobility, digitalized public services, and coherent infrastructure — not a lack of resources.
...
- 🏙️ Adopt the **"15-minute city" model** — ensure essential services (education, health, commerce, transport) are accessible within 15 minutes.
